...
| CI | Só validação documental (na branch F0/F1: também manifesto de contexto, registro de dívida, cobertura das regras obrigatórias e START_HERE sem número de PR nem checkpoint específico fora do Histórico); não roda solver nem benchmark; recomendação de pytest + `runner --check` em [CI_RECOMMENDATION_2026-09-12.md](CI_RECOMMENDATION_2026-09-12.md); proteção da main não configurada |
