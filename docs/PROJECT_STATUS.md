...
Painel reconciliado por fetch em 2026-09-25 (main `af247a7` = D16 investigado — sem mudança de código de produto, testes de caracterização + §75.1, revisão humana; antes `8df42be` = D6 / seção 80: verga e contraverga como reforço ESTRUTURAL da abertura nas duas opções, direto na main; antes `33960a0` rastreio de L + §76 (D6 investigado); antes `367eb0c` ciclo 3 (regra 79), `74cf2df` ciclo 2 (regra 49.1), `9376a36` ciclo 1 (regra 78), `ca014aa` runtime canônico e `4bb88bb` = PR #49 mesclado: motor do #42 + UI premium do #46 + OPÇÃO A da regra 48). SHAs são observações datadas.

```json
{
  "observed_utc": "2026-09-25T16:49:46+00:00",
  "main": "af247a73c7a4127571c6bb12bb876a8eff0c05bc",
  "official": [
    {
      "pr": 32,
...
| Área | Estado |
|---|---|
| MAIN / HEAD observado | `af247a73c7a4127571c6bb12bb876a8eff0c05bc` (D16 investigado, testes + docs, direto na main; antes `8df42be` = D6 / seção 80; antes `33960a0` = rastreio de L + §76; antes `367eb0c` = ciclo 3 / regra 79; antes `74cf2df` = ciclo 2 / regra 49.1; antes `9376a36` = ciclo 1 / regra 78; antes `ca014aa` = runtime canônico; `4bb88bb` = merge do **#49**; antes `55e990d` = #41; #40 CHANNEL em `61d4f6c`); antes 0e41c8e (#39), ad46c61 (#38), 6439669 (#37), 21576ee (#36), 6c00f7e (#35), aa58d70 (#32) |
| Solver oficial (main) | #37 (Beta 1 + scale-autofix: 8a.1, 11.10 revisada, 11.11, 11.13, 11.14, fileira de B34, filtro por layer (49), lote persistente (50)) + #38 (sonda de vão 3.1, licença 33.8, régua V2) + **#39** (paridade das amarrações encostadas, regra 33.9) + **#40** (estratégia de reforço de aberturas CHANNEL, regra 51). CHANNEL é **opt-in**: a Tela de Configuração abre em "Sem reforço adicional"; desde a **§80** (2026-09-25) verga e contraverga em canaleta existem nas duas opções e o CHANNEL é só a estratégia ADICIONAL. Falhas históricas do benchmark: TP1 V1 JUNCTION 8→9 e TGD V2 compensators 61→62 |
...
| **D16 — verga/contraverga que para no T (direto na main, 2026-09-25): REVISÃO HUMANA** | `af247a7`: **nenhuma mudança de código de produto.** Medido no BUTANTÃ: as 14 paradas do NONE (13 no CHANNEL) estão todas na amarração SELECIONADA do nó naquela fiada (13 B34 T_INTERSECTION_INCOMING + 1 B34 L_CORNER no L56); continuar colide (3 724 cm³ por T); travessias seguras = 0 (varredura de 515 configurações por parada; modelo A literal = 0). A única travessia é tirar a B34/B54 (51.6/51.7): T amarrados 37 → 35, gate 75 acusa. O HUMANO faz isso — 89/89 canaletas em nó sem B34/B54; 5 comparáveis ao D16 (T20/T26 c3/c11, T28 c11). Com amarração mantida o SCRIPT já está no apoio máximo físico. Decisão pendente: manter a 75 (6 lados −1 cm em revisão) × reativar a 51.6 (fiada de T sem amarração) × mover abertura (3B). Testes `tests/test_d16_canaleta_no_t.py`; REGRAS §75.1. [checkpoint](checkpoints/2026-09-25-d16-canaleta-no-t.md) |
...
| Próximo objetivo | Aguardando autorização (D9–D13 e D14 / Etapa 3B não iniciados; D16 investigado — destino da 51.6 é decisão do usuário); decisões E (topo/peitoril fora da grade) e F (cinta de topo); [pacote Beta 2](architecture/beta2-implementation-package.md) |
