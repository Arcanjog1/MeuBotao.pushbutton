...
> atualizações. Este arquivo é **append-only** — nunca apagar uma entrada
> anterior em silêncio.
>
> Este arquivo **não** é o estado operacional atual do projeto — isso é
> `docs/PROJECT_STATUS.md`. Ele também **não** substitui
> `nuvem/REGRAS_MODULACAO_BLOCOS.md` (regras técnicas de domínio).
>
> Contrato: `PROJECT_STATUS.md` = estado atual · `PROJECT_STATUS_LOG.md`
> (este arquivo) = histórico.
