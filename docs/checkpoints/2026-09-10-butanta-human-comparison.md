...
```json
{
  ...
  "pr": "not-created",
  "status": "EVIDÊNCIA MEDIDA AO VIVO + 3 CORREÇÕES GERAIS; regras novas listadas para confirmação do usuário",
  ...
  "regras_novas_candidatas_para_confirmacao": [
    ...
    "B34 COMO PEÇA CORRENTE (pendente, evidência estatística): preferir corridas de B34 a compensador para acertar o módulo. A prioridade da seção 2 já põe 'B39+B34' antes de compensador, mas o resultado medido (846 B34 / 739 C09 contra 1615 / 242) mostra que o gerador não chega lá - investigar `_pier_ordered_layout`/`MAX_COMPENSATORS_PER_TRECHO` com este corpus antes de mudar regra.",
