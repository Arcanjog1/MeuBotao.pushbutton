...
try:
    ...
    info = {"py": traceback.format_exc()}
    try:
        ce = getattr(e, "clsException", None)
        info["clr_type"] = str(ce.GetType().FullName)
        info["clr_stack"] = str(ce.StackTrace)
        inner = ce.InnerException
        if inner is not None:
            info["inner"] = str(inner.GetType().FullName) + " " + str(inner.StackTrace)
    ...
    try:
        ...
        while tb is not None:
            f = tb.tb_frame
            frames.append((f.f_code.co_name, tb.tb_lineno))
            last = f
            tb = tb.tb_next
        info["frames"] = frames
        loc = last.f_locals
        info["locals_keys"] = sorted(loc.keys())[:200]
        for key in ("wall_idx", "course", "variant_index", "variant_failed_spans", "opening_intervals_cm",
                    "boundaries", "active_boundaries", "extra_boundaries", "length_cm", "degraded_retry_done"):
            if key in loc:
                info["L_" + key] = repr(loc[key])[:2000]
    except Exception as e3:
        info["frames_err"] = str(e3)
    dump("q08_diag.json", info)
    raise
res = h.solve_result
