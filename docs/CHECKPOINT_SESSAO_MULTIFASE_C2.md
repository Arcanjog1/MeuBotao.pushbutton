...
1. **Normativa da CR-C2** — 3 opções medidas
   (`CR_C2_ROW_MOSTLY_EMPTY_WALL_SPLIT.md` §5). **Bloqueia** a
   implementação da C2. Recomendação registrada: **A + B**, se decidir agir.
2. **G12 residual** — aceitar 12 identidades críticas novas ou abrir CR.
...
Nada a fazer sem decisão humana no bloqueio (1). Trabalho independente
restante: nenhum dentro deste escopo.
