...
## 28. `CR-BLOCK-ARM-ROLE-INVARIANCE` — o papel `course_a`/`course_b` num
encontro L/T/X nunca pode fazer uma parede perder uma família inteira de
fiadas (2026-09-03)

### 28.1 REGRA OBRIGATÓRIA — o papel do braço num nó é bookkeeping, não
...
(seção 28.3 abaixo, custo já aceito) — nunca fazer uma parede ficar com
...
fill`) também falhava em fechar sozinho o trecho todo (ver 28.2), levando
...
- **Status**: DOCUMENTADO — fix parcial IMPLEMENTADO (ver 28.2). Testes
...
### 28.2 REGRA OBRIGATÓRIA — a fronteira "emprestada" de uma peça de nó
...
- **Escopo do fix, restrito de propósito** (medido ao vivo contra
  regressão real no benchmark, ver 28.4): só ativa quando (a) a parede
...
### 28.3 PADRÃO OBSERVADO, AINDA NÃO CORRIGIDO — troca de papel simétrica
...
o 28.1/28.2 exigem) é só espelhar qual parede desenha o padrão em qual
...
seções 28.1/28.2 só aparece quando (a) a mesma parede tem DOIS nós desse
...
### 28.4 CONFLITO CONHECIDO, NÃO RESOLVIDO — o fix de 28.2 recupera
...
| métrica | main (limpo) | com o fix (28.2) | delta |
|---|---|---|---|
...
abertura (rescatável pela regra 28.2) e outro com doadora COM abertura
(fora do escopo — ver 28.2, restrição (a)). O resultado: a família
