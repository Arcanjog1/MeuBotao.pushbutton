...
def project_paths(project_id, version=None):
    """Caminhos de UM projeto. `version=None` e' a regua historica V1 (raiz do
    projeto, arquivos com os nomes de sempre - nada muda para quem ja' chama
    isto). `version="v2"` (2026-09-12) aponta os arquivos GERADOS (snapshot
    da FASE A, input do solver, baseline/score/result) para o subdiretorio
    `<projeto>/<version>/`, mantendo os INSUMOS compartilhados na raiz:
    `input_real.json` (o problema medido), `reference.json` (o gabarito
    humano), `metadata.json`, `reference_score.json` e `evaluation_scope.json`
    (derivado so' do gabarito). Assim uma regua nova - topologia produzida
    pelo motor ATUAL - convive com a V1 sem sobrescrever nenhum historico."""
    directory = os.path.join(PROJECTS_DIR, project_id)
    generated = directory if not version else os.path.join(directory, version)
    return {
        "dir": directory,
        "version": version,
        "generated_dir": generated,
        "input": os.path.join(generated, "input.json"),
        "reference": os.path.join(directory, "reference.json"),
        "metadata": os.path.join(directory, "metadata.json"),
        "result": os.path.join(generated, "result.json"),
        "baseline": os.path.join(generated, "baseline.json"),
        ...
        "wall_modeling_snapshot": os.path.join(generated, "wall_modeling_snapshot.json"),
        ...
        "scoped_score": os.path.join(generated, "scoped_score.json"),
        "scoped_comparison": os.path.join(generated, "scoped_comparison.json"),
        "scope_summary": os.path.join(generated, "scope_summary.json"),
    }


def _report_name(project_id, version=None):
    return project_id if not version else "{0}__{1}".format(project_id, version)
...
def run_wall_modeling_only(project_id, write_files=True, version=None):
    ...
    esperar o pipeline inteiro. `version` (ver `project_paths`) grava o
    snapshot e o `input.json` derivado no subdiretorio da regua pedida."""
    from . import wall_modeling_bridge
    from .extract import wall_modeling_snapshot

    paths = project_paths(project_id, version)
    ...
    if write_files:
        if not os.path.isdir(paths["generated_dir"]):
            os.makedirs(paths["generated_dir"])   # subdiretorio de uma regua versionada
...
def run_scoped_evaluation(project_id, write_files=True, version=None):
    ...
    if write_files:
        ...
        _write_json(os.path.join(paths["generated_dir"], "scoped_reference_score.json"),
                    reference_score)
        text = report_module.full_report(score, findings, comparison, None,
                                         reference_score)
        if not os.path.isdir(REPORTS_DIR):
            os.makedirs(REPORTS_DIR)
        with open(os.path.join(REPORTS_DIR, "{0}_scoped.txt".format(_report_name(project_id, version))),
...
def run_project(project_id, save_baseline=False, write_files=True, version=None):
    """Roda o ciclo inteiro de um projeto. Devolve um dict com score,
    achados, comparacao e delta contra o baseline. `version` (ver
    `project_paths`) escolhe a regua: input/baseline/score do subdiretorio
    daquela versao, gabarito e piso de ruido compartilhados da raiz."""
    from . import solver_bridge
    from .extract import from_solver

    paths = project_paths(project_id, version)
    input_project = _read_json(paths["input"])
    input_used = paths["input"]
    if input_project is None and version:
        # Regua versionada SEM FASE A propria (projeto sem input_real.json,
        # ex.: TP1, cujo input e' reconstruido do gabarito): o problema e' o
        # MESMO da raiz - so' a medicao (baseline/score) e' da versao. Fica
        # registrado no resultado de onde o input veio, nunca implicito.
        input_used = project_paths(project_id)["input"]
        input_project = _read_json(input_used)
    ...
    result_project = from_solver.project_from_solver(
        ...
        metadata={"generated_at": datetime.datetime.now().isoformat(),
                  "from_input": os.path.relpath(input_used, PROJECTS_DIR),
                  "solver_notes": notes},
    )
    ...
    if write_files:
        ...
        _write_json(os.path.join(paths["generated_dir"], "findings.json"), findings)
        _write_json(os.path.join(paths["generated_dir"], "score.json"), score)
        if comparison is not None:
            _write_json(os.path.join(paths["generated_dir"], "comparison.json"), comparison)
        text = report_module.full_report(score, findings, comparison, delta,
                                         reference_score)
        if not os.path.isdir(REPORTS_DIR):
            os.makedirs(REPORTS_DIR)
        with open(os.path.join(REPORTS_DIR, "{0}.txt".format(_report_name(project_id, version))),
                  "w", encoding="utf-8") as handle:
            handle.write(text)
        if save_baseline:
...
def main(argv=None):
    ...
    parser.add_argument("--version", metavar="VERSION", default=None, dest="version",
                        help="regua alternativa (ex.: v2): input/snapshot/baseline/score em "
                             "<projeto>/<VERSION>/; sem isto, a regua historica V1 da raiz")
    ...
    if args.scoped:
        ...
        for project_id in (list_projects() if args.all else [args.run]):
            try:
                outcome = run_scoped_evaluation(project_id, version=version)
    ...
    if args.wall_modeling_only:
        ...
        for project_id in targets:
            try:
                snapshot = run_wall_modeling_only(project_id, version=version)
            except RuntimeError as exc:
                print("{0}: {1}".format(project_id, exc))
                continue
    ...
    if args.calibrate:
        ...
        return 0
    for project_id in targets:
        outcome = run_project(project_id, save_baseline=args.save_baseline, version=version)
        score = outcome["score"]
        if args.quiet:
