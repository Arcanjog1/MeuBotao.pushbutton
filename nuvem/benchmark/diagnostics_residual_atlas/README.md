...
## POST-PR19 revalidation (2026-09-06)

Este diretório foi copiado (sem alteração) da branch do Atlas
(`claude/block-solver-residual-root-cause-gwcmqa`, commit `9834b40`) para
`origin/main` (`3ebcd9b6`) como parte de
`docs/POST_PR19_ATLAS_INDEPENDENT_REVALIDATION.md`, para reexecutar as
medições do Atlas de forma independente sobre a árvore oficial atual.
Único arquivo NOVO (não existe na branch do Atlas):
`run_ablation_sweep.py` — varredura da matriz de tolerância 0.05-0.30cm
usada no item "C04 tolerance sweep" do relatório de revalidação.
